...
# python yolo_1.py --input /home/sandy/Desktop/project/opencv-dnn-gpu-examples/example_videos/d01_md_ch01_200222165514.mp4 --output yolo_sample2.avi --display 1 --use-gpu 1
...
firebase=firebase.FirebaseApplication("https://quickstart-1598859311837.firebaseio.com//quickstart-1598859311837/student",None)
...
while True:
	...
	if len(idxs) > 0:
		...
		for i in range(len(boxes)):
			if LABELS[classIDs[i]] in ("car", "truck", "bus", "person", "motorbike"):
				...
				data={
						...
						"geometry":{
							"type":"Point",
							"coordinates":[float(XYZ[0]),float(XYZ[1])]
						},
						"properties":{
							"SN":str(i+1),
							"Class_IDs":LABELS[classIDs[i]],
							"Time":C_time
						}
					}
				detail+=[data]
		...
		k += 1
		if k%50==0:
			result=firebase.post('/data/', detail)
			detail=[]
		clear()
